...
@routers.get("/", response_model=List[schemas.AttendanceOut])
...
@routers.post("/mark", response_model=schemas.AttendanceOut)
async def mark_attendance(attendance: schemas.AttendanceCreate, user=Depends(role_required([models.UserRole.teacher, models.UserRole.admin])), db: AsyncSession = Depends(get_db)):
